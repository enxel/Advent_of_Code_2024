...
class MinHeap {
    ...
    public:
        ...
        int count_ocurrences(int value, int index){
            if(index > heapsize) return 0;
            return ((heap[index] == value)?1:0) + count_ocurrences(value, left(index)) + count_ocurrences(value, right(index));
        }
};
...
int main(void){
    ...
    int acum = 0;
    for(int i = 0; i < L1.size(); i++)
        acum += L1.at(i) * h2.count_ocurrences(L1.at(i),0);

    int acum2 = 0;
    for(int i = 0; i <= limit; i++)
        acum2 += abs(h1.extract_min()-h2.extract_min());
